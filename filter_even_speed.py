...
def listofallsequences(args):
    ...
    incident, results5 = args  # Extract arguments
    sequences = []
    for sequence in results5[f'results5_{incident}.csv']['itemsets']:
...
def compute_relevance_for_incident(args):
    sequence, results5_file, df_i, len_df_i, sequences_by_incident = args  # Extract arguments
    h_in_value = 0
    ...
    if sequence in sequences_by_incident:
        ...
        h_in_value = results5_file['support'][sequences_by_incident.index(sequence)]
...
if __name__ == '__main__':
    ...
    data_alim['events + speed'] = data_alim['events + speed'].apply(ast.literal_eval)

    # Charger les résultats depuis le dossier results7
    results5 = {}
    for file in os.listdir('results/results5'):
        if file.endswith('.csv'):
            results5[file] = pd.read_csv('results/results5/' + file, sep=';')
            results5[file]['itemsets'] = results5[file]['itemsets'].apply(ast.literal_eval)
    ...
    args = [(incident, results5) for incident in incidents]
    ...
    events_summary = data_alim['events + speed']
    ...
    for incident in incidents:
        ...
        file = f'results5_{incident}.csv'
        results5_file = results5[file]
        len_df_i = len(df_i)
        
        pool_args3 = [(sequence, results5_file, df_i, len_df_i, sequences_by_incident[incident]) for sequence in sequences]
    ...
    for incident in incidents:
        with open(f'relevance\\event_speed\\Relevance_event_speed_{incident}_sequence.csv', 'w', newline='', encoding='utf-8') as csvfile:
