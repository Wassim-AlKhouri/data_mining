...
def process_incident_type(args):
    ...
    unique_events = set((event,speed, summary) for sequence in transactions for event,speed, summary in sequence)  # All unique events
    transaction_df = pd.DataFrame([
        {(event,speed, summary): (event,speed, summary) in sequence for event,speed, summary in unique_events} for sequence in transactions
    ])
    ...
    if not frequent_itemsets.empty:
        ...
        # Save results to CSV
        most_frequent.to_csv(f'results/results7/results7_{incident}.csv', sep=';', index=False)
...
def find_frequent_itemsets_fp_growth(data, min_support=MIN_SUPPORT):
    ...
    data['events + speed + alimentation'] = data['events + speed + alimentation'].apply(ast.literal_eval)
    ...
    transactions = data['events + speed + alimentation']
    unique_events = set((event, speed, summary) for sequence in transactions for event, speed, summary in sequence)  # All unique events
    transaction_df = pd.DataFrame([
        {(event,speed, summary): (event, speed, summary) in sequence for event, speed, summary in unique_events} for sequence in transactions
    ])
    ...
    database_frequent_itemsets.to_csv(f'results/results7/results_database7.csv', sep=';', index=False)
